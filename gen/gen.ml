...
module Func = struct
  ...
  let replace_map_zig =
    Map.of_alist_exn
      ...
      ; "cdist", "cdist_"
      ; "crow_indices", "crow_indices_"
      ; "ccol_indices", "ccol_indices_"
      ; "row_indices", "row_indices_"
      ; "col_indices", "col_indices_"
      ; "logsumexp", "logsumexp_"
      ; "any", "any_"
      ; "pdist", "pdist_"
      ; "is_coalesced", "is_coalesced_"
      ; "full", "full_"
      ; "abs", "abs_"
      ; "angle", "angle_"
      ; "size", "size_"
  ...
  let replace_fn_map_zig =
    Map.of_alist_exn
      ...
      ; "transpose_", "transpose2"
      ; "negative_", "negative2"
      ; "dropout_", "dropout2"
      ]

  let zig_name name map =
    let name =
      Map.find map name
      |> Option.value ~default:name
      |> String.lowercase
    ...
    let rec aux i uppercase_next =
      ...
      then Buffer.contents buffer
        ...
        then aux (i + 1) true
          ...
          let c' = if uppercase_next then Char.uppercase c else c in
          Buffer.add_char buffer c';
          aux (i + 1) false))
    in
    let result = aux 0 false in
    if len > 0 && Char.equal (String.get s (len - 1)) '_' then result ^ "_" else result
  ...
  let zig_binding_args t ~self =
    List.map t.args ~f:(fun arg ->
      let name =
        if Option.value_map self ~default:false ~f:(String.( = ) arg.arg_name)
        then "self"
        else zig_name arg.arg_name replace_map_zig
      in
      match arg.arg_type with
      | Tensor -> Printf.sprintf "%s.c_tensor" name
    ...
    let to_string args =
      List.map args ~f:(fun arg ->
        let zig_arg_type =
          ...
          | Scalar -> "Scalar"
          | ScalarType -> "Kind"
          | ScalarTypeOption -> "?Kind"
          | Device -> "Device"
        in
        Printf.sprintf "%s: %s" (zig_name arg.arg_name replace_map_zig) zig_arg_type)
      |> String.concat ~sep:", "
    in
    let self_arg =
      if String.is_suffix t.name ~suffix:"_" || String.( = ) t.name "set_data"
      then "self: *Tensor"
      else "self: *const Tensor"
    in
    match List.partition_tf t.args ~f:self_tensor with
    | [ self ], args_list ->
      Some self.arg_name, Printf.sprintf "%s, %s" self_arg (to_string args_list)
  ...
  let zig_return_type t =
    let returns =
      ...
      (*   |> Printf.sprintf "(%s)" *)
      (*   |> Option.some *)
      | `fixed v -> Some (Printf.sprintf "[%d]Tensor" v)
      | `dynamic -> Some "[]Tensor"
      | `bool -> Some "bool"
      | `int64_t -> Some "i64"
      | `double -> Some "f64"
    ...
    match returns with
    | None -> " void"
    | Some returns -> Printf.sprintf " %s" returns
end
...
let write_zig_wrapper funcs filename =
  (* Function to read the contents of a file into a string *)
  let read_file file_path =
    let content = In_channel.read_all file_path in
    String.sub content ~pos:0 ~len:(String.length content - 3)
  in
  (* Path to the header file *)
  let header_file_path = "tensor_partial.zig" in
  let header_contents = read_file header_file_path in
  Out_channel.with_file filename ~f:(fun out_ml ->
    let pm s = p out_ml s in
    pm "%s" header_contents;
    pm "    // Generated code starts here";
    Map.iteri funcs ~f:(fun ~key:exported_name ~data:(func : Func.t) ->
      let zig_name = Func.zig_name exported_name Func.replace_fn_map_zig in
      let camel_case_name = Func.snake_to_camel zig_name in
      let self, zig_args_list = Func.zig_typed_args_list func in
      pm "";
      pm "    pub fn %s(" camel_case_name;
      pm "        %s" zig_args_list;
      pm "    )%s {" (Func.zig_return_type func);
      List.iter func.args ~f:(fun arg ->
        match arg.arg_type with
        | DoubleOption | Int64Option ->
          () (* pm "        const %s = %s.into();" arg.arg_name arg.arg_name *)
        | _ -> ());
      match func.returns with
      | `dynamic ->
        pm
          "        const c_tensors = __c.atg_%s(%s);"
          exported_name
      ...
      | `nothing ->
        ...
        pm "    }"
      | `fixed ntensors ->
        pm "        var c_tensors = [_]C_tensor{null} ** %d;" ntensors;
        pm
...
let run
  ...
  write_zig_wrapper funcs zig_wrapper_filename

let () =
  run
    ~yaml_filename:"../pytorch/Declarations-v2.3.0.yaml"
    ~cpp_filename:"torch_api_generated"
    ~ffi_filename:"c_generated.rs"
    ~wrapper_filename:"tensor_generated.rs"
    ~fallible_wrapper_filename:"tensor_fallible_generated.rs"
    ~zig_wrapper_filename:"../src/tensor.zig"
